...
public class PartyDBServiceImpl implements PartyDBService {
    ...
    public int getPartyId(FighterBL fighterBL) {
        ...
        return template.queryForObject("SELECT Id from main5.party where Name = '" + fighterBL.getParty() + "'", Integer.class);

    }

    public String getPartyName(int partyId) {

        return template.queryForObject("SELECT Name from main5.party where Id = '" + partyId + "'", String.class);
